...
Last updated: 2026-06-03 (**P1 COMPLETE** — Task 15 verified live read+write against `nnash`; Plan 3a Awareness #20; green polish #17/#19)
...
- **Next:** P1 is done. Remaining roadmap — **Plan 3 (rest):** Links, Tags, KeyDates + reminders, scheduled export/backup job (3a Awareness already shipped #20). Then **P2 — M365** (gated on the Entra recon spike, risk R3). Then **P3 — AI & decks**. Optional Task-15 nicety: exercise the dossier endpoint on a couple of seeded real `nnash` records (mock-vs-real field gaps to watch: booleans as `"true"/"false"`, choice/date formats, pagination).
...
- **Plan 3 — Awareness (remaining):** ✅ 3a activity timeline + stale-client radar (D16); still to do — Links, Tags, KeyDates + reminders, scheduled export/backup job.
