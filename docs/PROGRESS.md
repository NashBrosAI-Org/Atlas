...
- **Desktop:** Plan A (desktop shell) complete on `feature/desktop-app` — `Atlas.app` builds via `scripts/build-desktop.sh` and runs as a native window on demo data; serves UI + `/api` from one local process, no Gatekeeper quarantine (D13).
